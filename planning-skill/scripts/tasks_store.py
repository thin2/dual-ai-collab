...
def _normalize_task_num(task_num: str) -> str:
    normalized = str(int(task_num or "0")) if str(task_num or "").isdigit() else str(task_num).lstrip("0")
...
def _extract_commands_from_text(text: str) -> list[str]:
    ...
    for line in text.splitlines():
        ...
        if in_fenced_block:
            continue
        commands.extend(match.strip() for match in re.findall(r"`([^`]+)`", line))
    return commands
...
def task_lock_path(task_file: str | Path = DEFAULT_TASKS_MD) -> Path:
...
def resolve_task_paths(task_file: str | Path = DEFAULT_TASKS_MD) -> TaskPaths:
    ...
    if raw_str in {"", DEFAULT_TASKS_MD, DEFAULT_TASKS_JSON}:
        return TaskPaths(
            markdown_path=_resolve_workspace_path(Path(DEFAULT_TASKS_MD)),
            json_path=_resolve_workspace_path(Path(DEFAULT_TASKS_JSON)),
        )
    raise CliError(f"任务文件路径必须是项目内的 .md 或 .json 文件: {task_file}")
...
def normalize_document(document: dict, paths: TaskPaths) -> dict:
    ...
    normalized["version"] = _validate_document_version(document.get("version", DOCUMENT_VERSION))
...
def _validate_document_version(value: object) -> int:
    try:
        version = int(value)
    except (TypeError, ValueError) as exc:
        raise CliError(f"任务文档 version 非法: {value!r}") from exc
    if version != DOCUMENT_VERSION:
        raise CliError(f"不支持的任务文档 version: {version}（当前支持 {DOCUMENT_VERSION}）")
    return version
...
def render_markdown(document: dict) -> str:
    ...
    for task in document.get("tasks", []):
        ...
        lines.extend(_render_task_fields(task))
...
def _render_task_fields(task: dict) -> list[str]:
    ...
    for default_key in ("priority", "status", "assigned_to", "dependencies", "execution_level", "stall_threshold_minutes"):
        if default_key in seen:
            continue
        line = _render_task_field(task, default_key)
        if line:
            rendered.append(line)
    return rendered


def _render_task_field(task: dict, entry: str) -> str | None:
    if entry.startswith("extra:"):
        label = entry.split(":", 1)[1]
        if label in task["extra_fields"]:
            return f"**{label}**: {task['extra_fields'][label]}"
        return None
...
def read_tasks_document(task_file: str | Path = DEFAULT_TASKS_MD, *, lock_cache: bool = True) -> dict:
    paths = resolve_task_paths(task_file)
    if paths.json_path.exists():
        return _read_json_document(paths)
    ...
    with FileLock(task_lock_path(paths.markdown_path)):
        if paths.json_path.exists():
            return _read_json_document(paths)
        markdown = paths.markdown_path.read_text(encoding="utf-8")
        document = parse_markdown(markdown, paths)
...
def _read_json_document(paths: TaskPaths) -> dict:
    try:
    ...
    if not isinstance(data, dict):
        raise CliError(f"任务 JSON 根节点必须是对象: {paths.json_path}")
